...
def _docs_reference_section(config: dict[str, Any]) -> str:
    ...
    return (
        ...
        "Directions are standalone Markdown files containing precise instructions for a specific recurring task — written so that another LLM can read and execute them independently (e.g. a scheduled agent).\n\n"
        "**When to create a directions file:**\n"
        "- User says 'speicher das als Direktive', 'leg eine Directions-Datei an', 'merk dir wie das geht', 'speichere diese Anweisung' or similar\n"
        "- A scheduled task prompt references a directions file (e.g. 'read directions/github-issues-check.md')\n"
        "- A task is complex enough that encoding it fully in a schedule prompt would be unwieldy\n\n"
        f"**Format:** `{config.get('agent_dir', 'agent_dir')}/directions/descriptive-name.md` — plain Markdown, self-contained.\n"
        "**What to include:**\n"
        ...
        "- GraphQL queries must be a single-line escaped string — no newlines\n"
        "- Brace expansion `{0..6}` only works in bash script files, NOT in single-line exec calls — use `for i in 0 1 2 3 4 5 6` instead\n"
        "- Only save commands that have been verified to work — no drafts, no intermediate attempts\n"
        "- Parse JSON responses with `jq`, not string manipulation\n\n"
        "**When to READ a directions file:** When a prompt says 'Lies die Anweisungen aus directions/...' or similar — read the file and follow it exactly.\n"
    )
